...
export function parseFailureDiagnostics(message: string | null | undefined): FailureDiagnostics | null {
  ...
  const summary =
    detailsTokenIndex >= 0
      ...
      : guidanceTokenIndex >= 0
        ...
        : message.trim()
  const guidance =
    guidanceTokenIndex >= 0 ? message.slice(guidanceTokenIndex + ' Next check: '.length).trim() : null
  ...
  const detailsLength =
    guidanceTokenIndex > detailsStart ? guidanceTokenIndex - detailsStart : message.length - detailsStart
...
function parseDetails(detailsSegment: string): FailureDiagnosticItem[] {
  ...
  for (let index = 0; index < matches.length; index += 1) {
    ...
    const nextIndex =
      index + 1 < matches.length ? (matches[index + 1].index ?? detailsSegment.length) : detailsSegment.length
