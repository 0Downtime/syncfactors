...
public sealed class ExternalSystemExceptionFactoryTests
{
    ...
    public void CreateActiveDirectoryTimeoutException_PreservesMessageAndMarksRetryMetadata()
    {
        ...
        var exception = Assert.IsType<InvalidOperationException>(
            method!.Invoke(null, ["lookup", "qadc-01.spireqa.biz", TimeSpan.FromSeconds(10), new TimeoutException("Timed out.")]));

        Assert.Equal("Active Directory lookup timed out against LDAP server 'qadc-01.spireqa.biz' after 10 seconds.", exception.Message);
    ...
    public void CreateActiveDirectoryException_ReportsMissingOuGuidance_ForNoObjectDirectoryOperation()
    {
        ...
        Assert.Contains("Best match in AD was 'OU=SpireQA-Users,DC=spireQA,DC=biz'.", exception.Message, StringComparison.Ordinal);
    ...
    public void CreateActiveDirectoryException_ForCreateUserNoObject_IncludesTargetDnAndRawAdMessage()
    {
        ...
        var config = new ActiveDirectoryConfig(
            Server: "localhost",
            Port: 636,
            Username: "svc_successfactors@spireqa.biz",
            BindPassword: "secret",
            IdentityAttribute: "employeeID",
            DefaultActiveOu: "OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz",
            PrehireOu: "OU=Prehire,OU=SpireQA-Users,DC=spireQA,DC=biz",
        ...
        var ldapException = new LdapException(
            32,
            "The object does not exist. 0000208D: NameErr: DSID-0310028D, problem 2001 (NO_OBJECT), data 0, best match of:\n\t'OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz'");
        var details = "Step=CreateUser WorkerId=30008382 SamAccountName=30008382 DistinguishedName=CN=30008382,OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz TargetOu=OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz";

        var exception = Assert.IsType<InvalidOperationException>(method!.Invoke(null, ["command 'CreateUser'", config, ldapException, details]));

        Assert.Contains("Active Directory reported NO_OBJECT while creating 'CN=30008382,OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz' under target OU 'OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz'.", exception.Message, StringComparison.Ordinal);
        Assert.Contains("Best match in AD was 'OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz'.", exception.Message, StringComparison.Ordinal);
        Assert.Contains("Raw AD error: The object does not exist.", exception.Message, StringComparison.Ordinal);
        Assert.Contains("Current create target DN='CN=30008382,OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz', targetOu='OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz'.", exception.Message, StringComparison.Ordinal);
    }
    ...
    public void CreateActiveDirectoryException_ForCreateUserPreflightSearchNoObject_IncludesTargetDnAndRawAdMessage()
    {
        ...
        var ldapException = new LdapException(
            32,
            "The object does not exist. 0000208D: NameErr: DSID-0310028D, problem 2001 (NO_OBJECT), data 0, best match of:\n\t'OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz'");
        var details = "Step=PreflightIdentityConflictSearch WorkerId=30008382 SamAccountName=30008382 DistinguishedName=CN=30008382,OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz TargetOu=OU=POWERSHELL,OU=SpireQA-Users,DC=spireQA,DC=biz";
