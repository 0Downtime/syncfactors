...
public sealed class SuccessFactorsWorkerSourceIntegrationTests
{
    ...
    public async Task WorkerSource_ListWorkersAsync_FollowsServerSidePaginationLinks()
    {
        ...
        var scaffoldStore = new ScaffoldDataStore(new ScaffoldDataPathResolver(scaffoldDataPath));
        var fallbackSource = new ScaffoldWorkerSource(scaffoldStore);
        var workerSource = new SuccessFactorsWorkerSource(client, configLoader, new DisabledDeltaSyncService(), fallbackSource, logger);

        var workers = new List<WorkerSnapshot>();
        await foreach (var worker in workerSource.ListWorkersAsync(WorkerListingMode.DeltaPreferred, CancellationToken.None))
        {
            workers.Add(worker);
            if (workers.Count == 5)
            {
    ...
    [Fact]
    public async Task WorkerSource_ListWorkersAsync_DeltaPreferred_AppendsDeltaFilter()
    {
        var fixturePath = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "config", "mock-successfactors", "baseline-fixtures.json"));
        var fixtureStore = new MockFixtureStore(Options.Create(new MockSuccessFactorsOptions
        {
        ...
        var scaffoldStore = new ScaffoldDataStore(new ScaffoldDataPathResolver(scaffoldDataPath));
        var fallbackSource = new ScaffoldWorkerSource(scaffoldStore);
        var workerSource = new SuccessFactorsWorkerSource(
            client,
            configLoader,
        ...
        Assert.Contains(
            handler.RequestUris
                .Select(uri => Microsoft.AspNetCore.WebUtilities.QueryHelpers.ParseQuery(new Uri(uri).Query))
                .Select(query => query.TryGetValue("$filter", out var filter) ? filter.ToString() : null),
            filter => !string.IsNullOrWhiteSpace(filter) &&
                      filter.Contains("status eq 'active'", StringComparison.Ordinal) &&
                      filter.Contains("lastModifiedDateTime ge datetimeoffset'2026-03-30T12:00:00Z'", StringComparison.Ordinal));
        Assert.Contains(logger.Messages, message => message.Contains("using SuccessFactors delta sync", StringComparison.OrdinalIgnoreCase));
    }
    ...
    [Fact]
    public async Task WorkerSource_GetWorkerAsync_BridgesPreviewIdentityBackToProvisioningIdentity()
    {
        using var client = new HttpClient(new BridgedPreviewIdentityHttpHandler())
        {
            BaseAddress = new Uri("http://mock-successfactors.local")
        };
    ...
    [Fact]
    public async Task WorkerSource_GetWorkerAsync_UsesTargetedPreviewBridge_BeforeFullPreviewLookup()
    {
        var handler = new TargetedPreviewBridgeHttpHandler();
        using var client = new HttpClient(handler)
        {
            BaseAddress = new Uri("http://mock-successfactors.local")
        };
        ...
        var worker = await workerSource.GetWorkerAsync("u10001", CancellationToken.None);

        Assert.NotNull(worker);
        Assert.Equal("u10001", worker!.WorkerId);
        Assert.Equal("p10001", worker.Attributes["personIdExternal"]);
    ...
    [Fact]
    public async Task ListWorkersAsync_PreservesCanonicalEmploymentStatus_WhenPreviewOmitsStatus()
    {
        using var client = new HttpClient(new PreviewMissingEmploymentStatusHttpHandler())
        {
            BaseAddress = new Uri("http://mock-successfactors.local")
        };
    ...
    [Fact]
    public async Task ListWorkersAsync_ReadsEmploymentStatusFromTopLevelEmpJobFields()
    {
        using var client = new HttpClient(new TopLevelEmpJobEmploymentStatusHttpHandler())
        {
            BaseAddress = new Uri("http://mock-successfactors.local")
        };
    ...
    private sealed class DisabledDeltaSyncService : IDeltaSyncService
    {
    ...
    private sealed class PreviewMissingEmploymentStatusHttpHandler : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            _ = cancellationToken;

            if (request.RequestUri?.AbsolutePath.Equals("/odata/v2/EmpJob", StringComparison.OrdinalIgnoreCase) == true)
            {
                const string empJobJson = """
                {
                  "d": {
                    "results": [
                      {
    ...
    private sealed class TopLevelEmpJobEmploymentStatusHttpHandler : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            _ = cancellationToken;
    ...
    private sealed class BridgedPreviewIdentityHttpHandler : HttpMessageHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            _ = cancellationToken;
    ...
    private sealed class TargetedPreviewBridgeHttpHandler : HttpMessageHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (request.RequestUri.AbsolutePath.Equals("/odata/v2/EmpJob", StringComparison.OrdinalIgnoreCase) &&
                filter.Contains("userId eq 'u10001'", StringComparison.Ordinal))
            {
                return Task.FromResult(JsonResponse(
                    """
                    {
            ...
            if (request.RequestUri.AbsolutePath.Equals("/odata/v2/PerPerson", StringComparison.OrdinalIgnoreCase) &&
                filter.Contains("personIdExternal eq 'u10001'", StringComparison.Ordinal))
            {
                return Task.FromResult(JsonResponse("""{"d":{"results":[]}}"""));
            }

            if (request.RequestUri.AbsolutePath.Equals("/odata/v2/PerPerson", StringComparison.OrdinalIgnoreCase) &&
                filter.Contains("employmentNav/userId eq 'u10001'", StringComparison.Ordinal))
            {
                return Task.FromResult(JsonResponse(
                    """
                    {
